  ...
  try {
    ...
    channelJobs.consume(consumerName, message => {
      ...
      if (input.consumerName == consumerName) {
        console.log(
          `Recieved job with start date ${input.firstDate} and end date ${input.lastDate}`
        );
        channelJobs.ack(message);
        handleCountRequest([input.firstDate, input.lastDate], result => {
          console.log(result);
          channelJobs.assertQueue("CONSUMER_3_RESULT");
          channelJobs.sendToQueue(
            "CONSUMER_3_RESULT",
            Buffer.from(JSON.stringify({ result: result }))
          );
        });
      }
    });
